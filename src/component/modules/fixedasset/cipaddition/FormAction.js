...
const ROUTES = '/fixedasset/Trx/cipaddition'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch cipaddition')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch cipadditiondetail')

    const response = await eplant.get(`${ROUTES}/detail?cipcode=${(_.isUndefined(params[0]) ? '' : params[0])}&period=${format(params[1], 'mm')}&year=${format(params[1], 'yyyy')}`)
    console.log('done fetch cipadditiondetail')
