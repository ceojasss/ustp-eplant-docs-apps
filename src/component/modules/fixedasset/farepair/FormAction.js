...
const ROUTES = '/fixedasset/Trx/farepair'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch farepair')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch farepairdetail')

    const response = await eplant.get(`${ROUTES}/detail?referencenumber=${(_.isUndefined(params[0]) ? '' : params[0])}&period=${format(params[1], 'mm/yyyy')}`)

    console.log('done fetch farepairdetail')
