...
const ROUTES = '/fixedasset/Trx/maturitystatement'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch maturitystatement')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch maturitystatementdetail')

    const response = await eplant.get(`${ROUTES}/detail?jobcode=${(_.isUndefined(params[0]) ? '' : params[0])}&period=${format(params[1], 'mm/yyyy')}`)

    console.log('done fetch maturitystatementdetail')
