...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Realisasi SPD',
        btnIcon: 'pen square',
        addClickHandler: () => {
            dispatch(ShowData(SPD))
        },
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.no_spd))
                    await dispatch(fetchDatadetail(getRowData(row)));

                await PrepareEditDataGrid([row.original.no_spd, row.original.empname])

                navigate(`./edit/${encodeURIComponent(row.original.no_spd)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.no_spd} Akan Dihapus`, row))
                break;
            default:
                break;
        }
    ...
    //  const getarray = [row.original.no_spd, dateperiode]
    ...
    const renderRowSubComponent = React.useCallback(
        ...
        ({ row, rowProps, visibleColumns }) => (
            ...
            <SubRowAsync
                 ...
                 */
                //row={[row.original.no_spd, dateperiode]}
                row={getRowData(row)}
                rowProps={rowProps}
