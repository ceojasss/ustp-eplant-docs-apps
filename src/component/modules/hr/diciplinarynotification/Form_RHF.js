...
const customschema = (formValidationSchema, formComps, methods) => {
    ...
    yup.addMethod(yup.array, 'autoIncident', function (fields) {
        return this.test('autoIncident', 'autoIncident', function (array, val) {
            ...
            _.mapValues(val.originalValue, (v, index) => {
                ...
                // console.log(methods)
                if(_.isNil(v.incident_seq) && _.isNil(v.rowid)){
                    setValue(`${this.path}[${index}].incident_seq`, x, { shouldDirty: true })
                    // console.log(index)
                    errors.push(index)
                } 
                // else {
