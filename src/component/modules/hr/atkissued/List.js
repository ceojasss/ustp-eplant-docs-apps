...
import { EMPLOYEE, TGLATK } from "../../../Constants"
...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Input Data ATK Issued',
        btnIcon: 'pen square',
        addClickHandler: () => {
            dispatch(ShowData(EMPLOYEE))
        }
    }
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                //console.log(_.some(datadetail, ['empcode', 'CM-EX1199']))
                dispatch({ type: SET_LOADING_STATUS, payload: true })

                if (!isDataExist(row.original.empcode))
                    await dispatch(fetchDatadetail(getRowData(row)));

                await PrepareEditDataGrid([row.original.empcode, row.original.description])

                navigate(`./edit/${encodeURIComponent(row.original.empcode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.empcode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
    ...
    //  const getarray = [row.original.empcode, dateperiode]
    ...
    const renderRowSubComponent = React.useCallback(
        ({ row, rowProps, visibleColumns }) => (
            <SubRowAsync
                 ...
                 */
                //row={[row.original.empcode, dateperiode]}
                row={getRowData(row)}
                rowProps={rowProps}
    ...
    return (
        <ContentHeader
            ...
            <RenderTable
                ...
                data={UpdateResultHeader(data[0])}
