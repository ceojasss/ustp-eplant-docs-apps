...
export const fetchDatadetail = (params, callback) => async dispatch => {
    ...
    const response = await eplant.get(`${ROUTES}/detail?empcode=${(_.isUndefined(params[0]) ? '' : params[0])}&period=${format(params[1], 'mm/yyyy')}&period2=${format(params[2], 'mm/yyyy')}&period3=${format(params[3], 'mm/yyyy')}`)
