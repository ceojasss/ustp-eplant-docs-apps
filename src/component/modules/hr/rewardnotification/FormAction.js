...
const ROUTES = '/hr/Trx/rewardnotification'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    // console.log('done fetch rewardnotification')
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    // console.log('run fetch languagedetail')

    const response = await eplant.get(`${ROUTES}/detail?nik_staff=${params}`)

    // console.log('done fetch languagedetail')
