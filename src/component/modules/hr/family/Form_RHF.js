...
const customschema = (formValidationSchema, formComps, methods) => {
    ...
    yup.addMethod(yup.string, "setstatmed",
    function setstatmed() {
        return this.test((value, values) => {
            ...
            if (!_.isUndefined(getValues('inputgrid[0]'))){
                ...
                for (let i = 0; i < _.size(getValues('inputgrid')); i++) {
                                    // console.log('2',getValues(`inputgrid[${i}].stat_med`))
                                    if (_.isNull(getValues(`inputgrid[${i}].stat_med`)) ){

                                        setValue(`inputgrid[${i}].stat_med`,'0' , { shouldDirty: true })
                                    } 
                }
    ...
    if (formValidationSchema) {
        customSchema = formValidationSchema.concat(
            yup.object().shape({
                setstatmed: yup.string().setstatmed(),
            })
        )
    }
