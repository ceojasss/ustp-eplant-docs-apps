    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                // // console.log('1',Appresources.BUTTON_LABEL.LABEL_LANJUT_TRX)
                ...
                // // console.log( _.remove(row.original,{rowid:row.original.rowid}))
                    dispatch(setActionModals(Appresources.BUTTON_LABEL.LABEL_LANJUT_TRX, Object.values(row.original)))
                    dispatch(fetchDatadetail(row.original.nik_staff, async (x) => {
                        // console.log('2',x)
                        if (_.isEmpty(x)) {
