...
const List = ({ data, datadetail, isloading, queryPageSize, dateperiode, queryPageIndex, pageCount, search, reducerid }) => {
    // console.log(data[0])
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.docnum))
                    await dispatch(fetchDatadetail(row.original.docnum,
                        // async () => {
                ...
                await prepareEditDataMasterDetail(row)
                navigate(`./edit/${encodeURIComponent(row.original.docnum)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.docnum} Akan Dihapus`, row))
                break;
            default:
                break;
        }
