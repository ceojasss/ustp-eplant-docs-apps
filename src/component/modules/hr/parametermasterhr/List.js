    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                // _.without(row.original,row.original.rowid)
                // console.log( _.remove(row.original,{rowid:row.original.rowid}))
                    dispatch(setActionModals(null, Object.values(row.original)))
                    dispatch(fetchDatadetail(row.original.parametercode, async (x) => {
                        console.log('2',x)
                        if (_.isEmpty(x)) {
                        //     console.log('3', selectedData)
                            ...
                            dispatch({ type: SET_LOADING_STATUS, payload: true })
                            await PrepareEditDataGrid([row.original.parametercode, row.original.parametername])
                ...
                // await PrepareEditDataGrid([row.original.parametercode, row.original.parametervalue])
