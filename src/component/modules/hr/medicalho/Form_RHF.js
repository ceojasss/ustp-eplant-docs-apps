...
import { updateData, createData, process_document } from "./FormAction";
...
const Form = ({ title, actions, formComp, initialValues, postdata, formValidationSchema, submitdata, actionlabel, resetTrx, urlpreview }) => {
    ...
    const pa_process_message = 'Proses Permintaan Anggaran'
    ...
    const handlerReport = async () => {
        ...
        if (_.isEmpty(getValues('pa_number'))) {
            dispatch({
                type: MODAL_STATE,
                payload: {
                    state: true,
                    content: 'Konfirmasi Transaksi.?',
                    message: pa_process_message,
    ...
    if (getValues('documentno') !== 'autonumber') {

        buttonReport = [{
            btnTitle: (_.isEmpty(getValues('pa_number')) ? 'Process Permintaan Anggaran' : 'Cetak Permintaan Anggaran'),
            btnIcon: 'inbox',
            color: 'red',
            btnBasic: false,
    ...
    useEffect(() => {
        ...
        if (actions) {
            ...
            if (actions === pa_process_message) {
                dispatch(process_document(getValues(), ({ returnvalues }) => {

                    let retVal = returnvalues?.outBinds?.panumber
                    if (retVal !== 'FAILED') {
                        setValue('pa_number', retVal, { shouldDirty: true })
                    }
...
const customschema = (formValidationSchema, formComp, formComps, methods, title) => {
    ...
    const totalbiaya = (v) => {
        ...
        setValue('biaya', val, { shouldDirty: true })
        /*
