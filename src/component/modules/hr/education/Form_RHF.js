...
const Forms = ({ title, periode, units, actions, formComps, initialValues, postdata, formValidationSchema, resetTrx, submitdata, actionlabel }) => {
    ...
    const defaultDataValue = { nik_staff: units[0] }
...
yup.addMethod(yup.array, 'seqLevel', function (fields) {
    return this.test('seqLevel', 'seqLevel', function (array, val) {
        ...
        _.mapValues(val.originalValue, (v, index) => {
            ...
            // console.log(methods)
            if(_.isNil(v.seqlevel) && _.isNil(v.rowid)){
                setValue(`${this.path}[${index}].seqlevel`, '0', { shouldDirty: true })
                // console.log(index)
                errors.push(index)
            } 
            if(_.isNil(v.fg_abroad) && _.isNil(v.rowid)){
                setValue(`${this.path}[${index}].fg_abroad`, '0', { shouldDirty: true })
                // console.log(index)
                errors.push(index)
            } 
            // else {
...
const mapStateToProps = (state) => {
    ...
    console.log(state.auth.modals.actionpick)
    return {
