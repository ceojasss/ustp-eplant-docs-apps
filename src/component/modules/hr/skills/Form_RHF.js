...
const customschema = (formValidationSchema, formComps, methods) => {
    ...
    yup.addMethod(yup.array, 'autoSeq', function (fields) {
        return this.test('autoSeq', 'autoSeq', function (array, val) {
            ...
            _.mapValues(val.originalValue, (v, index) => {
                ...
                // console.log(methods)
                if(_.isNil(v.seq) && _.isNil(v.rowid)){
                    setValue(`${this.path}[${index}].seq`, x, { shouldDirty: true })
                    // console.log(index)
                    errors.push(index)
                } 
                // else {
