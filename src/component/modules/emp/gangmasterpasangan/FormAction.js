...
const ROUTES = '/emp/Setup/gangmasterpasangan'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch gangmasterpasangan')
...
export const fetchDatadetail = (params) => async dispatch => {
    ...
    const response = await eplant.get(`${ROUTES}/detail?gangcode=${(_.isUndefined(params[0]) ? '' : params[0])}&month=${format(params[1], 'mm')}&year=${format(params[1], 'yyyy')}`)

    console.log('done fetch gangmasterpasangandetail')
