...
const ROUTES = "/emp/Trx/employeeadhocallowancededuction";
export const fetchDataheader =
  (pageIndex, pageSize, search, date, callback) => async (dispatch) => {
    ...
    //    const response = await eplant.get(`/cashbank/Trx/employeeadhocallowancededuction?page=${(_.isUndefined(pageIndex) ? 0 : pageIndex)}&size=${(_.isUndefined(pageSize) ? 10 : pageSize)}&search=${search}`)
    ...
    console.log("done fetch employeeadhocallowancededuction");
...
export const fetchDatadetail = (params) => async (dispatch) => {
  console.log("run fetch employee-adhoc-allowance-deductiondetail");
  
  const response = await eplant.get(`${ROUTES}/detail?allowdedcode=${params[0]}&period=${format(
    params[1],
    "mm/yyyy"
  )}`);
  console.log(response)

  console.log("done fetch employee-adhoc-allowance-deductiondetail");
