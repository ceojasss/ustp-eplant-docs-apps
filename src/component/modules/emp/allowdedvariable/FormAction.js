...
const ROUTES = '/emp/setup/allowancevariable'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch allowancevariable')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch allowancevariabledetail')

    const response = await eplant.get(`${ROUTES}/detail?allowdedcode=${(_.isUndefined(params) ? '' : params)}`)

    console.log('done fetch allowancevariabledetail')
