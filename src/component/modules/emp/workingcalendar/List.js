...
import { fetchDataheader, fetchDatadetail, deleteData, fetchDetailEdit } from './FormAction'
import {
...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Input Data Working Calendar',
        btnIcon: 'pen square',
        addClickHandler: () => {
            dispatch(ShowData(WC))
        },
        // disabled: IsInsert(data) != 'Y' ? true : false
        // disabled: !_.isUndefined(data[0]) ? (_.get(data[1][0], 'startdate')) <= new Date(dateperiode).toLocaleDateString('en-GB') ?  IsInsert(data) == 'Y' ? false : true : true : 'null'
    }
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                //// console.log(_.some(datadetail, ['pyear', 'CM-EX1199']))
                dispatch({ type: SET_LOADING_STATUS, payload: true })

                if (!isDataExist(row.original.pyear))
                    await dispatch(fetchDetailEdit(getRowData(row)));

                await PrepareEditDataGrid([row.original.pyear, row.original.pmonth])

                navigate(`./edit/${encodeURIComponent(row.original.pyear)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.pyear} Akan Dihapus`, row))
                break;
            default:
                break;
        }
    ...
    //  const getarray = [row.original.pyear, dateperiode]
    ...
    const renderRowSubComponent = React.useCallback(
        ({ row, rowProps, visibleColumns }) => (
            <SubRowAsync
                 ...
                 */
                //row={[row.original.pyear, dateperiode]}
                row={getRowData(row)}
                rowProps={rowProps}
    ...
    useEffect(() => {
        ...
        switch (crudActions) {
            ...
            case Appresources.BUTTON_LABEL.LABEL_LANJUT_TRX:
                ...
                dispatch(fetchDetailEdit([selectedData[0], dateperiode], async (x) => {
                    if (_.isEmpty(x)) {
