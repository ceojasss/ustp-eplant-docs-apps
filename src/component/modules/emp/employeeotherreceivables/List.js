    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                await dispatch(fetchDatadetail(row.original.emparid,
                    async () => {
                        await prepareEditDataMasterDetail(row)
                    })
                );

                navigate(`./edit/${encodeURIComponent(row.original.emparid)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.emparid} Akan Dihapus`, row))
                break;
            default:
                break;
        }
