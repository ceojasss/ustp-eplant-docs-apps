...
const ROUTES = '/emp/Setup/setupgangmaster'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch setupgangmaster')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch setupgangmasterdetail')

    const response = await eplant.get(`${ROUTES}/detail?departmentcode=${(_.isUndefined(params) ? '' : params)}`)

    console.log('done fetch setupgangmasterdetail')
