...
const Forms = ({ title, periode, units, actions, formComp, formComps, initialValues, postdata, formValidationSchema, submitdata, actionlabel, resetTrx,urlpreview }) => {
    ...
    useEffect(() => {
        ...
        if (resetTrx) {
            ...
            if (!_.isEmpty(submitdata)) {
                ...
                setTimeout(() => {
                    ...
                    if (_.isUndefined(initialValues['v_url_preview'])) {
                        ...
                        // } else {
                        dispatch(fetchLovUrlPreview('NR', initialValues['process_flag'], initialValues['gangcode']))
                        // }
                    }
                }, 0.1);
            } else {
            ...
            // // console.log(doctype)
            // if (doctype == 'MRSLR' ){
            //     dispatch(fetchLovUrlPreview(doctype,initialValues['process_flag'],initialValues['gangcode']))
            // } else {
            //     dispatch(fetchLovUrlPreview('MR',initialValues['process_flag'],initialValues['gangcode']))
            // }
            // }
            // console.log('hahaha')
            // dispatch(fetchLovUrlPreview(doctype,initialValues['process_flag'],initialValues['gangcode']))
