...
const ROUTES = "/emp/Trx/employeereceivables";
export const fetchDataheader =
  (pageIndex, pageSize, search, date, callback) => async (dispatch) => {
    ...
    //    const response = await eplant.get(`/cashbank/Trx/employeereceivables?page=${(_.isUndefined(pageIndex) ? 0 : pageIndex)}&size=${(_.isUndefined(pageSize) ? 10 : pageSize)}&search=${search}`)
    ...
    console.log("done fetch employeereceivables");
...
export const fetchDatadetail = (params) => async (dispatch) => {
  console.log("run fetch employeereceivablesdetail");

  const response = await eplant.get(`${ROUTES}/detail?emparid=${params}`);

  console.log("done fetch employeereceivablesdetail");
