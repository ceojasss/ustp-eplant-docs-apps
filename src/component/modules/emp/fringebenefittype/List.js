...
/*=============================================================================
 ...
 |
 |          
 |  Description:  List Fringe Benefit Type
 |
 |
 |
 |
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                await dispatch(fetchDatadetail(row.original.emptype,
                    async () => {
                        await prepareEditDataMasterDetail(row)
                    })
                );

                navigate(`./edit/${encodeURIComponent(row.original.emptype)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.emptype} Akan Dihapus`, row))
                break;
            default:
                break;
        }
