    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                await dispatch(fetchDatadetail(row.original.jvno,
                    async () => {
                        await prepareEditDataMasterDetail(row)
                    })
                );

                navigate(`./edit/${encodeURIComponent(row.original.jvno)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.jvno} Akan Dihapus`, row))
                break;
            default:
                break;
        }
