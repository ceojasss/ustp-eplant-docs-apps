/*=============================================================================
 ...
 |                - Bagas Dwi Putra
 |          
 |  Description:  List Transaksi Debit Note
 |
 *===========================================================================*/
     ...
     const rowClickHandler = async (action, row) => {
         ...
         switch (action) {
             case UPDATE:
                 ...
                 if (!isDataExist(row.original.drnoteid))
                     await dispatch(fetchDatadetail(row.original.drnoteid
                         // async () => {
                 ...
                 navigate(`./edit/${encodeURIComponent(row.original.drnoteid)}`)
 
                 break;
             case DELETE:
                 // * set custom delete message     
                 dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.drnoteid} Akan Dihapus`, row))
                 break;
             default:
                 break;
         }
