...
export const ROUTES = '/journalvoucher/trx/debitnote'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    // console.log('done fetch debitnote')
...
export const fetchDatadetail = (params,callback) => async dispatch => {
    console.log('run fetch debit note')

    const response = await eplant.get(`${ROUTES}/detail?drnoteid=${(_.isUndefined(params) ? '' : params)}`)

    console.log('done fetch debitnotedetail')
