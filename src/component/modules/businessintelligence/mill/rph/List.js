...
const Lists = ({  data, p_year, p_month, title ='Rekapan Produksi Harian'  }) => {

  const dispatch = useDispatch();


  useEffect(() => {
    dispatch(fetchData(p_year,p_month,));
  }, [dispatch ,p_year,p_month]);
  ...
  return (
    <RenderTable
      columns={getColumnMill(data,'componentrph')}
      data={data[0]}
      title={title}
      pagination={true}
    />
