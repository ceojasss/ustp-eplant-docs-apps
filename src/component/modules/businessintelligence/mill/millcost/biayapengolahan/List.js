...
const Lists = ({dataheader,datadetail,p_year,p_site,title = "Biaya Pengolahan",}) => {
  ...
  useEffect(() => {
    if (dataheader && dataheader.component) {
      const filteredColumns = filteringTableMill(dataheader.component,"componenttabbiayaolah");
  ...
  const handleCellClick = (rowIndex, columnIndex) => {
    if (showDetail) {
      ...
      setBreadcrumbTrail(prevTrail => [
        ...prevTrail,
        { column: "Detail Biaya Pengolahan" }
      ]);
    } else {
  ...
  useEffect(() => {
    if (datadetail && datadetail.component) {
      const filteredColumns = filteringTableMill(
        datadetail.component,"componentdetailolah");
...
const mapStateToProps = (state) => {
  ...
  return {
    dataheader: state.businessintelligence.fetchbiayaolah,
    datadetail: state.businessintelligence.fetchbiayaolahdetail,
  };
};
