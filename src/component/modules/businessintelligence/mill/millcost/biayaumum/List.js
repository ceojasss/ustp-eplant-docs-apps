...
const Lists = ({dataheader,datadetail,p_year,p_site,title = "Alokasi Biaya Umum",}) => {
  ...
  useEffect(() => {
    if (dataheader && dataheader.component) {
      const filteredColumns = filteringTableMill(dataheader.component,"componentbiayaumum");
  ...
  const handleCellClick = (rowIndex, columnIndex) => {
    if (showDetail) {
      ...
      setBreadcrumbTrail(prevTrail => [
        ...prevTrail,
        { column: "Detail ALokasi Biaya Umum" }
      ]);
    } else {
  ...
  useEffect(() => {
    if (datadetail && datadetail.component) {
      const filteredColumns = filteringTableMill(
        datadetail.component,"componentdetailumum");
...
const mapStateToProps = (state) => {
  ...
  return {
    dataheader: state.businessintelligence.fetchbiayaumum,
    datadetail: state.businessintelligence.fetchbiayaumumdetail,
  };
};
