...
import { deleteData, fetchDatadetail, fetchDataheader } from './FormAction'
...
const List = ({ data, datadetail, isloading, queryPageSize, queryPageIndex, pageCount, search, dateperiode, selectedData, crudActions, reducerid }) => {
    ...
    // // console.log(_.get(_.find(data[1]),'isinsert'))
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                navigate(`./edit/${encodeURIComponent(row.original.wocompletionno)}`)
                break;
            case DELETE:
