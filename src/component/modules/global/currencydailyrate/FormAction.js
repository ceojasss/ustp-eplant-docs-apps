...
const ROUTES = '/generalsetup/Setup/currencydailyrate'
...
export const fetchDatadetail = (params,callback) => async dispatch => {
    console.log('run fetch currencytldetail')

    const response = await eplant.get(`${ROUTES}/detail?currid=${params[0]}&period=${format(params[1], 'mm/yyyy')}`)

    console.log('done fetch currencytldetail')
