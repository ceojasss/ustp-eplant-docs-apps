    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
                ...
                // console.log( _.remove(row.original,{rowid:row.original.rowid}))
                    ...
                    dispatch(fetchDatadetail(getRowData(row), async (x) => {
                        ...
                        //     console.log('3', selectedData)
                            ...
                            dispatch({ type: SET_LOADING_STATUS, payload: true })
                            await PrepareEditDataGrid([row.original.currid, row.original.currdesc])
                            navigate(`./edit/${encodeURIComponent(row.original.currid)}`)
                        }
                    }))
                // if (!isDataExist(row.original.nik_staff))
            ...
            case DELETE:
                ...
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.currid} Akan Dihapus`, row))
                break;
            default:
                break;
        }
