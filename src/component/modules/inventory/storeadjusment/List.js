     ...
     const rowClickHandler = async (action, row) => {
         ...
         switch (action) {
             case UPDATE:
                 ...
                 if (!isDataExist(row.original['voucher#']))
                    await dispatch(fetchDatadetail(row.original['voucher#'],
                        // async () => {
                ...
                await prepareEditDataMasterDetail(row)
                 navigate(`./edit/${encodeURIComponent(row.original['voucher#'])}`)
                 break;
             case DELETE:
                 // * set custom delete message     
                 dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original['voucher#']} Akan Dihapus`, row))
                 break;
             default:
                 break;
         }
