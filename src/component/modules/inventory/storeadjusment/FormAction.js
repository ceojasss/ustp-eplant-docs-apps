...
const ROUTES = '/stores/Trx/storeadjusment'
...
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    //console.log('done fetch store adjusment')
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    console.log('run fetch storeadjusmentdetail')

    const response = await eplant.get(`${ROUTES}/detail?${encodeURIComponent('voucher#')}=${params}`)

    console.log('done fetch storeadjusmentdetail')
