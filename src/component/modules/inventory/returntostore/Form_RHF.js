                ...
                setTimeout(() => {
                    ...
                    if (_.isUndefined(initialValues['v_url_preview'])) {
                        // if (_.isEmpty(submitdata['inputgrid'])){
                            dispatch(fetchLovUrlPreview('RTN', initialValues['process_flag'], initialValues['returnnotenumber']))
                    }
                }, 0.1);
            } else {
 ...
 */                    break;
    ...
    let buttonReport
        ...
        if (actionlabel == Appresources.BUTTON_LABEL.LABEL_UPDATE && (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('rpt_returntostore_new.rdf') : _.includes(initialValues['v_url_preview'], 'rpt_returntostore_new.rdf')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_RETURNTOSTORE_NEW.rdf') : _.includes(initialValues['v_url_preview'], 'RPT_RETURNTOSTORE_NEW.rdf')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_RETURNTOSTORE_NEW.RDF') : _.includes(initialValues['v_url_preview'], 'RPT_RETURNTOSTORE_NEW.RDF'))) 
    {
        // console.log('1')
        buttonReport = [{
