 ...
 *===========================================================================*/
 ...
 const Forms = ({ title, periode, units, actions, formComp, formComps, initialValues, postdata, formValidationSchema,
    ...
    const generateData = async (cb) => {
        ...
        const key_doc = _.get(methods.control._formValues.transferincode,'code')
        const date = methods.control._formValues.wodatedisplayonly
        const transferreceivecode = methods.control._formValues.transferreceivecode
        ...
        if (!_.isEmpty(key_doc) && _.size(methods.control._formValues.inputgrid) === 0) {
            ...
            await dispatch(fetchDataGenerate(key_doc,date,transferreceivecode, () => {
            ...
            if (!_.isEmpty(submitdata)) {
                ...
                setTimeout(() => {
                    ...
                    if (_.isUndefined(initialValues['v_url_preview'])) {
                        ...
                        // } else {
                        dispatch(fetchLovUrlPreview('TRI', initialValues['process_flag'], initialValues['transferreceivecode']))
                        // }
                    }
                }, 0.1);
            } else {
    ...
    if (actionlabel == Appresources.BUTTON_LABEL.LABEL_UPDATE && (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('rpt_transferreceive_slip.rdf') : _.includes(initialValues['v_url_preview'], 'rpt_transferreceive_slip.rdf')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_TRANSFERRECEIVE_SLIP.rdf') : _.includes(initialValues['v_url_preview'], 'RPT_TRANSFERRECEIVE_SLIP.rdf')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_TRANSFERRECEIVE_SLIP.RDF') : _.includes(initialValues['v_url_preview'], 'RPT_TRANSFERRECEIVE_SLIP.RDF'))) {
        // console.log('1')
        buttonReport = [{
...
const customschema = (formValidationSchema, formComp, formComps, methods) => {
    ...
    yup.addMethod(yup.string, "setqty",
        function authDate() {
            return this.test((value, values) => {


                // console.log(getValues('inputgrid[0]'))
if (!_.isUndefined(getValues('inputgrid[0]'))){
//     qty_receiveddisplayonly
// : 
// 0
// qtytransferdisplayonly
// : 
// 10
// quantityreceive
// : 
// undefined
                //     // console.log(_.size(getValues('inputgrid')))
                    for (let i = 0; i < _.size(getValues('inputgrid')); i++) {
                        let amount =  getValues(`inputgrid[${i}].qty_receiveddisplayonly`)+getValues(`inputgrid[${i}].quantityreceive`)
                        if (amount >= getValues(`inputgrid[${i}].qtytransferdisplayonly`)){
                            if (getValues(`inputgrid[${i}].qty_receiveddisplayonly`) != 0) {
                                amount = getValues(`inputgrid[${i}].qtytransferdisplayonly`)-getValues(`inputgrid[${i}].qty_receiveddisplayonly`)
                                // console.log('1',amount)
                                setValue(`inputgrid[${i}].quantityreceive`, amount, { shouldDirty: true })
                            } else {
                                // console.log('2',getValues(`inputgrid[${i}].qtytransferdisplayonly`))
                                setValue(`inputgrid[${i}].quantityreceive`,getValues(`inputgrid[${i}].qtytransferdisplayonly`) , { shouldDirty: true })
                            }
                        } else {
                            // console.log('3',getValues(`inputgrid[${i}].quantityreceive`))
                            setValue(`inputgrid[${i}].quantityreceive`, getValues(`inputgrid[${i}].quantityreceive`), { shouldDirty: true })
                        }
