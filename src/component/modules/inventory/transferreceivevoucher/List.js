    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.transferreceivecode))
                    await dispatch(fetchDatadetail(row.original.transferreceivecode,row.original.transferreceivedate,row.original.transferincode
                        // async () => {
                ...
                await prepareEditDataMasterDetail(row)
                navigate(`./edit/${encodeURIComponent(row.original.transferreceivecode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.transferreceivecode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
