...
const ROUTES = "/stores/trx/wipstockledger";
...
export const fetchDataheader =
  (pageIndex, pageSize, search, date, callback) => async (dispatch) => {
    ...
    const response = await eplant.get(
      `${ROUTES}?page=${pageIndex}&size=${pageSize}`
    );
...
export const fetchDatadetail = (params, callback) => async (dispatch) => {
  ...
  const response = await eplant.get(
    `${ROUTES}/detail?parametervaluecode=${
      _.isUndefined(params[0]) ? "" : params[0]
    }&month=${format(params[1], "mm")}&year=${format(params[1], "yyyy")}`
  );
