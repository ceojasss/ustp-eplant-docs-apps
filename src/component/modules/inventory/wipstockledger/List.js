...
import { PARAMETERVALUE, VH } from "../../../Constants"
...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Input Data Vehicle Activity',
        btnIcon: 'pen square',
        addClickHandler: () => {
            dispatch(ShowData(PARAMETERVALUE, null, 'WIP_GROUP'))
        },
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.parametervaluecode))
                    await dispatch(fetchDatadetail(getRowData(row)));

                await PrepareEditDataGrid([getRowData(row)])

                navigate(`./edit/${encodeURIComponent(row.original.parametervaluecode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.parametervaluecode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
    ...
    useEffect(() => {
        ...
        initData(dispatch, queryPageIndex, queryPageSize, dateperiode, loc.pathname, reducerid)

    }, [dispatch, reducerid, loc.pathname, queryPageIndex, queryPageSize, dateperiode])
...
const initData = (dispatch, queryPageIndex, queryPageSize, dateperiode, paths, reducerid) => {

    changeReducer(paths.replaceAll('/', ''), reducer)

    if (paths.replaceAll('/', '') === reducerid)
        dispatch(fetchDataheader(queryPageIndex, queryPageSize, dateperiode))
}
