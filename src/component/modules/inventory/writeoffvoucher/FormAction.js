...
const ROUTES = '/stores/Trx/writeoffvoucher'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch writeoffvoucher')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch writeoffvoucherdetail')

    const response = await eplant.get(`${ROUTES}/detail?writeoffcode=${params}`)

    console.log('done fetch writeoffvoucherdetail')
