...
const ROUTES = '/stores/Trx/transfervoucher'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    //console.log('done fetch transfervoucher')
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    console.log('run fetch transfervoucherdetail')

    const response = await eplant.get(`${ROUTES}/detail?transfercode=${params}`)

    console.log('done fetch transfervoucherdetail')
