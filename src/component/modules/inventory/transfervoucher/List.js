/*=============================================================================
 ...
 |                
 |          
 |  Description:  - List Transfer Voucher
 |                               
 |
 *===========================================================================*/
...
import { isDataExist } from "../../../../utils/DataHelper"
  ...
  const List = ({ data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid, isloading }) => {
      // console.log(data[0])
     ...
     const rowClickHandler = async (action, row) => {
         ...
         switch (action) {
             case UPDATE:
                 ...
                 if (!isDataExist(row.original.transfercode))
                    await dispatch(fetchDatadetail(row.original.transfercode,
                        // async () => {
             ...
             await prepareEditDataMasterDetail(row)
                 navigate(`./edit/${encodeURIComponent(row.original.transfercode)}`)
                 break;
             case DELETE:
                 // * set custom delete message     
                 dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.transfercode} Akan Dihapus`, row))
                 break;
             default:
                 break;
         }
