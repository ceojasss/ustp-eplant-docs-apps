...
const ROUTES = '/fieldoperation/Trx/plantingcompletion'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch plantingcompletion')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch plantingcompletiondetail')

    const response = await eplant.get(`${ROUTES}/detail?documentno=${params}`)

    console.log('done fetch plantingcompletiondetail')
