...
const ROUTES = '/fieldoperation/Trx/classblocknew'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch classblocknew')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch classblocknewdetail')

    const response = await eplant.get(`${ROUTES}/detail?estatecode=${params[0]}&divisioncode=${params[1]}&fieldcode=${params[2]}&period=${format(params[3], 'mm/yyyy')}`)

    console.log('done fetch classblocknewdetail')
