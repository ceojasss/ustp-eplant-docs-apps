...
const customschema = (formValidationSchema, formComps, methods) => {
    ...
    yup.addMethod(yup.string, "setSampleBox",
        function setSampleBox() {
            return this.test('setSampleBox', 'Berat Spikelet Sample 1 + 2',(value, values) => {

                // console.log(getValues('authorized'))
                // setValue('authorized', '0', { shouldDirty: true })
                // console.log(getValues('berat_spikelet_sample_1'))
                if (!_.isUndefined(getValues('berat_spikelet_sample_1'))){
                    // getValues('berat_spikelet_sample_1')
                    setValue('beratspikeletdisplayonly', getValues('berat_spikelet_sample_1') + getValues('berat_spikelet_sample_2'), { shouldDirty: true })
                }
                return true
    ...
    customSchema = formValidationSchema
        .concat(
            yup.object().shape({
                sample: yup.string().setSampleBox(),
            }))
