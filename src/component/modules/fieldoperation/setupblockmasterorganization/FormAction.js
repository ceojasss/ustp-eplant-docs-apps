...
const ROUTES = '/fieldoperation/Setup/blockorganization'
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    ...
    const response = await eplant.get(`${ROUTES}/detail?estatecode=${(_.isUndefined(params[0]) ? '' : params[0])}&divisioncode=${(_.isUndefined(params[1]) ? '' : params[1])}`)
