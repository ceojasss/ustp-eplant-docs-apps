...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Input Data Master Organization',
        btnIcon: 'pen square',
        addClickHandler: () => {
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                // console.log(row)
                // console.log(row.original.departmentcode)
                if (!isDataExist(row.original.divisioncode))
                    await dispatch(fetchDatadetail(getRowData(row)));
                    
                await PrepareEditDataGrid([row.original.divisioncode,row.original.estatecode,row.original.concessionid])

                navigate(`./edit/${encodeURIComponent(row.original.divisioncode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.divisioncode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
    ...
    useEffect(() => {
        ...
        switch (crudActions) {
            ...
            case Appresources.BUTTON_LABEL.LABEL_LANJUT_TRX:
                ...
                dispatch(fetchDatadetail([selectedData[1],selectedData[0]], async (x) => {
                    if (_.isEmpty(x)) {
                        console.log('3', selectedData)
                        navigate("./new")
                    } else {
                        dispatch({ type: SET_LOADING_STATUS, payload: true })
                        await PrepareEditDataGrid([selectedData[0],selectedData[1],selectedData[2]])
