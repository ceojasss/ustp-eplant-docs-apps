...
const ROUTES = '/fieldoperation/Trx/progresslc'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch progresslc')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch progresslcdetail')

    const response = await eplant.get(`${ROUTES}/detail?blockid=${params[0]}&period=${format(params[1], 'mm/yyyy')}`)

    console.log('done fetch progresslcdetail')
