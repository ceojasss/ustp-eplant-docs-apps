...
import { isDataExist } from "../../../../utils/DataHelper"
 ...
 const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
     ...
     const button = {
         btnLabel: 'Input Data Daily Planting',
         btnIcon: 'pen square',
         addClickHandler: () => {
             dispatch(ShowData(EA))
         }
     }
     ...
     const rowClickHandler = async (action, row) => {
         ...
         switch (action) {
             case UPDATE:
                 ...
                 if (!isDataExist(row.original.kebun))
                    await dispatch(fetchDatadetail(getRowData(row)));
                 
                 await PrepareEditDataGrid([row.original.kebun, row.original.afdeling])
 
                 navigate(`./edit/${encodeURIComponent(row.original.kebun)}`)
                 break;
             case DELETE:
                 // * set custom delete message     
                 dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.kebun} Akan Dihapus`, row))
                 break;
             default:
                 break;
         }
     ...
     //  const getarray = [row.original.kebun, dateperiode]
     ...
     const renderRowSubComponent = React.useCallback(
         ({ row, rowProps, visibleColumns }) => (
             <SubRowAsync
                  ...
                  */
                 //row={[row.original.kebun, dateperiode]}
                 row={getRowData(row)}
                 rowProps={rowProps}
