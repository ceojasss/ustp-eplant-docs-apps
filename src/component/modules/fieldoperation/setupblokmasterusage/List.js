...
import { BMU } from "../../../Constants"
import { Appresources } from "../../../templates/ApplicationResources"
import { DELETE_DATA, UPDATE_NAV_PERIOD, UPDATE_NAV_SEARCH,RESET_DETAIL_DATA, RESET_MODAL_ONLY, RESET_ERROR_MODAL_STATE, DELETE, UPDATE, SET_LOADING_STATUS } from "../../../../redux/actions/types"
...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Input Data Block Master Usage',
        btnIcon: 'pen square',
        addClickHandler: () => {
            dispatch(ShowData(BMU))
        },
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.blockid, row.original.concessionid))
                    await dispatch(fetchDatadetail(getRowData(row)));
                // console.log(row.original.concessionid)
                await PrepareEditDataGrid([row.original.blockid, row.original.concessionid])
        ...
        switch (crudActions) {
            ...
            case Appresources.BUTTON_LABEL.LABEL_DELETE:
                ...
                case Appresources.BUTTON_LABEL.LABEL_LANJUT_TRX:
                    ...
                    dispatch(fetchDatadetail([selectedData[1]], async (x) => {
                        if (_.isEmpty(x)) {
                            console.log('3',selectedData)
                            navigate("./new")
                        } else  {
                            dispatch({ type: SET_LOADING_STATUS, payload: true })
                            await PrepareEditDataGrid([selectedData[1], selectedData[0]])
                            navigate(`./edit/${encodeURIComponent(selectedData[1])}`)
                        }
                    }));
