...
const ROUTES = '/fieldoperation/Trx/supplying'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch supplying')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch supplyingdetail')

    const response = await eplant.get(`${ROUTES}/detail?kebun=${params[0]}&period=${format(params[1], 'mm/yyyy')}`)

    console.log('done fetch supplyingdetail')
