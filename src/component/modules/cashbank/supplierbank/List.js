...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Input Data Supplier Bank',
        btnIcon: 'pen square',
        // addClickHandler: () => {
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.suppliercode))
                    await dispatch(fetchDatadetail(getRowData(row)));

                await PrepareEditDataGrid([row.original.suppliercode, row.original.suppliername])

                navigate(`./edit/${encodeURIComponent(row.original.suppliercode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.suppliercode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
