...
import { getFormListComponent, useYupValidationResolver, getFormComponent, InitValidationMixed, isFieldLov, getTitle, getFormTitle, UserInfo } from "../../../../utils/FormComponentsHelpler";
...
const Forms = ({ title, periode, units, actions, formComp, formComps, initialValues, postdata, formValidationSchema, submitdata, actionlabel, resetTrx, urlpreview }) => {
            ...
            if (!_.isEmpty(submitdata)) {
                ...
                setTimeout(() => {
                    ...
                    if (_.isUndefined(initialValues['v_url_preview'])) {
                        ...
                        // }
                        // // console.log(doctype)
                        doctype = UserInfo().loginid.match(/^.*HO$/) ? 'RV' : 'RVSO'
                        ...
                        // // console.log(doctype)
                        if (doctype == 'RV') {
                            dispatch(fetchLovUrlPreview(doctype, initialValues['process_flag'], initialValues['vouchercode']))
                        } else {
                            dispatch(fetchLovUrlPreview('RVSO', initialValues['process_flag'], initialValues['vouchercode']))
                        }
                        // dispatch(fetchLovUrlPreview('RV', initialValues['process_flag'], initialValues['vouchercode']))
                    }
                }, 0.1);
    ...
    let buttonReport
    if (actionlabel == Appresources.BUTTON_LABEL.LABEL_UPDATE && (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_RECEIVE_VOUCHER.RDF') : _.includes(initialValues['v_url_preview'], 'RPT_RECEIVE_VOUCHER.RDF')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_RECEIVE_VOUCHER.RDF') : _.includes(initialValues['v_url_preview'], 'RPT_RECEIVE_VOUCHER.RDF')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_RECEIVE_VOUCHER.rdf') : _.includes(initialValues['v_url_preview'], 'RPT_RECEIVE_VOUCHER.rdf'))) {
        // // console.log('1')
        buttonReport = [{
        ...
        if (actionlabel == Appresources.BUTTON_LABEL.LABEL_UPDATE && (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('rpt_receive_voucher_so.rdf') : _.includes(initialValues['v_url_preview'], 'rpt_receive_voucher_so.rdf')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_RECEIVE_VOUCHER_SO.RDF') : _.includes(initialValues['v_url_preview'], 'RPT_RECEIVE_VOUCHER_SO.RDF'))) {
            // // console.log('2')
