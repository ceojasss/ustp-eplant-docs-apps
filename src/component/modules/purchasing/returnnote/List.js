    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.returnnotenumber))
                    await dispatch(fetchDatadetail(row.original.returnnotenumber,
                        // async () => {
                ...
                navigate(`./edit/${encodeURIComponent(row.original.returnnotenumber)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.returnnotenumber} Akan Dihapus`, row))
                break;
            default:
                break;
        }
