...
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    // console.log('done fetch returnnote')
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    // console.log('run fetch returnnotedetail')

    const response = await eplant.get(`${ROUTES}/detail?returnnotenumber=${(_.isUndefined(params) ? '' : params)}`)

    // console.log('done fetch returnnotedetail')
