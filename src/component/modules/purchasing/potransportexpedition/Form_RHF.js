...
const Forms = ({ title, periode, units, actions, formComp, formComps, initialValues, postdata, formValidationSchema, submitdata, actionlabel, resetTrx, urlpreview, busy }) => {
            ...
            if (!_.isEmpty(submitdata)) {
                ...
                setTimeout(() => {
                    ...
                    if (_.isUndefined(initialValues['v_url_preview'])) {
                        ...
                        // if (doctype == 'POSLR' ){
                        dispatch(fetchLovUrlPreviewArray('PA', initialValues['documentcode'], initialValues['potcode']))
                        dispatch(fetchLovUrlPreviewArray('POT', initialValues['documentcode'], initialValues['documentcode']))
                        // } else {
                        // }
                    }
                }, 0.1);
    ...
    const handlerReportSolar = () => {
        // // console.log(_.get(urlpreview[0],'v_url_preview'))
        if (!_.isUndefined(initialValues['v_url_preview_list'])) {
            // // console.log('hehe',initialValues['v_url_preview_solar'])
            window.open(initialValues['v_url_preview_list'])
        } else {
    ...
    if (actionlabel == Appresources.BUTTON_LABEL.LABEL_UPDATE && (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPTPP5.RDF') : _.includes(initialValues['v_url_preview'], 'RPTPP5.RDF')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[1], 'v_url_preview').match('rptpp5.RDF') : _.includes(initialValues['v_url_preview'], 'rptpp5.RDF'))) {
        // // console.log('1')
        buttonReport = [{
            ...
            btnTitle: 'View Report List Data',
            btnIcon: 'file pdf outline',
