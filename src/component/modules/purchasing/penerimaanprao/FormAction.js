...
const ROUTES = '/purchasing/Trx/penerimaanprao'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    // console.log('done fetch penerimaanprao')
...
export const fetchDatadetail = (params) => async dispatch => {
    // console.log('run fetch penerimaanpraodetail')

    const response = await eplant.get(`${ROUTES}/detail?prcode=${(_.isUndefined(params) ? '' : params)}`)

    // console.log('done fetch penerimaanpraodetail')
