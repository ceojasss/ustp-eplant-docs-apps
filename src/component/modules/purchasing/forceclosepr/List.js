...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Input Data Force Close PR',
        btnIcon: 'pen square',
        addClickHandler: () => {
            dispatch(ShowData(LPR))
        }
    }
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.prcode))
                    await dispatch(fetchDatadetail(getRowData(row)));

                await PrepareEditDataGrid([row.original.prcode, row.original.prnotes])

                navigate(`./edit/${encodeURIComponent(row.original.prcode)}`)
                break;
            default:
                break;
        }
