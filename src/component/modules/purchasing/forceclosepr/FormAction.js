...
const ROUTES = '/purchasing/Trx/forceclosepr'
...
export const fetchData = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    // console.log('done fetch ForceClosePR')
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    ...
    const response = await eplant.get(`${ROUTES}/detail?prcode=${(_.isUndefined(params[0]) ? '' : params[0])}&dateperiode=${format(params[1], 'mm/yyyy')}`)
