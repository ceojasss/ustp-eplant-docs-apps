...
const ROUTES = '/purchasing/Trx/evalsupplier'
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    // console.log('run fetch evalsupplier')
    
    const response = await eplant.get(`${ROUTES}/detail?suppliercode=${params[0]}&tdate=${params[1]}`)

    // console.log('done fetch evalsupplier')
...
export const fetchDataGenerate = (params, progressno, progressdate, callback) => async dispatch => {
    ...
    const response = await eplant.get(`${ROUTES}/crdata`)
