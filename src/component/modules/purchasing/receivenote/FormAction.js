...
export const ROUTES = '/purchasing/Trx/receivenote'
...
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    // console.log('done fetch receivenote')
...
export const fetchDatadetail = (params) => async dispatch => {
    // console.log('run fetch receivenotedetail')

    const response = await eplant.get(`${ROUTES}/detail?receivenotecode=${(_.isUndefined(params) ? '' : params)}`)

    // console.log('done fetch receivenotedetail')
