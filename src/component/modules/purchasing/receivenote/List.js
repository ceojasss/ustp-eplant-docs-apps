...
import { changeReducer, getColumn, getColumnDetail, getDetailData, getRowData, getTitle, IsInsert, periodHandler, prepareEditDataMasterDetail, QueryData, QueryDataDetail, QueryDatePeriode, QueryPageCount, QueryPageIndexes, QueryPageSizes, QueryReducerID, QuerySearch, searchHandler } from "../../../../utils/FormComponentsHelpler"
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.receivenotecode))
                    await dispatch(fetchDatadetail(row.original.receivenotecode,
                        // async () => {
                ...
                navigate(`./edit/${encodeURIComponent(row.original.receivenotecode)}`)

                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.receivenotecode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
