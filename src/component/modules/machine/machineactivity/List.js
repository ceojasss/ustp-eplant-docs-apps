...
const List = ({ crudActions, selectedData, isloading, data, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const button = {
        btnLabel: 'Input Data Machine Activity',
        btnIcon: 'pen square',
        addClickHandler: () => {
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.machinecode))
                    await dispatch(fetchDatadetail(getRowData(row)));

                await PrepareEditDataGrid([row.original.machinecode, row.original.description])

                navigate(`./edit/${encodeURIComponent(row.original.machinecode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.machinecode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
