...
import {
    ...
    QueryDatePeriode, QueryPageCount, QueryPageIndexes, QueryPageSizes, QueryReducerID, QuerySearch, searchHandler, prepareEditDataMasterDetail, QuerySelectedData
...
import { isDataExist } from "../../../../utils/DataHelper"
 ...
 |                
 |          
 |  Description:  - List Transfer Receive Voucher
 |                               
 |
 *===========================================================================*/
 ...
 const List = ({ data, crudActions, selectedData, datadetail, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid  }) => {
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.vehiclegroupcode))
                    await dispatch(fetchDatadetail(row.original.vehiclegroupcode,row.original.period
                ...
                await prepareEditDataMasterDetail(row)
                navigate(`./edit/${encodeURIComponent(row.original.vehiclegroupcode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.vehiclegroupcode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
    ...
    const onDispatchdetail = (params) => {
        // dispatch(fetchDatadetail(params[0],params[1],params[2]))
