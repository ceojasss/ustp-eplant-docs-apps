...
const ROUTES = '/vehicle/Trx/vehicleactivity'
...
export const fetchDetailBydate = (params, callback) => async dispatch => {
    ...
    const dateSelected = !_.isNil(st.auth.modals.actionvalueone) ? st.auth.modals.actionvalueone : new Date(st[trxKeys()].datefilterone)
    const dateSelected2 = !_.isNil(st.auth.modals.actionvaluetwo) ? st.auth.modals.actionvaluetwo : new Date(st[trxKeys()].datefiltertwo)
...
// console.log(st.auth.modals.actionvaluetwo)
    ...
    const response = await eplant.get(`${ROUTES}/detailbydate?vehiclecode=${(_.isUndefined(params[0]) ? '' : params[0])}&periodone=${parseDatetoString(dateSelected)}&periodtwo=${parseDatetoString(dateSelected2)}`)
