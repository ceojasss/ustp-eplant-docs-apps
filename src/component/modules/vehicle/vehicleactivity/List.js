    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.vehiclecode))
                    await dispatch(fetchDetailEdit(getRowData(row)));

                await PrepareEditDataGrid([row.original.vehiclecode, row.original.description])

                navigate(`./edit/${encodeURIComponent(row.original.vehiclecode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.vehiclecode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
