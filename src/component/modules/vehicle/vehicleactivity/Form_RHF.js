...
import { updateData, createData, fetchDetailBydate, executeMAVH } from "./FormAction";
...
const Forms = ({ title, periode, units, actions, formComps, initialValues, postdata, formValidationSchema, resetTrx, submitdata, actionlabel }) => {
  ...
  */
        ...
        if (actions) {
            if (actions === Appresources.TRANSACTION_ALERT.CHANGE_DATE_FILTER) {
                ...
                dispatch(fetchDetailBydate(units, v => {
                    ...
                    if (!_.isNil(st.auth.modals.actionvalueone)) {

                        dispatch({ type: DATE_FILTER, payload: { datefilterone: st.auth.modals.actionvalueone } })
                        dispatch({
                            type: CHANGE_MODAL_ITEM_STATE,
                            payload: { actionpick: null, actionvalueone: null, actionvaluetwo: null }
                        })
                    } else
                        if (!_.isNil(st.auth.modals.actionvaluetwo)) {

                            dispatch({ type: DATE_FILTER, payload: { datefiltertwo: st.auth.modals.actionvaluetwo } })
                            dispatch({
                                type: CHANGE_MODAL_ITEM_STATE,
                                payload: { actionpick: null, actionvalueone: null, actionvaluetwo: null }
                            })
                ...
                switch (actions) {
                    ...
                    case Appresources.BUTTON_LABEL.LABEL_SAVE:
                        ...
                        dispatch(executeMAVH(_.merge(defaultDataValue,{'p_month':Number(dateFormat(periode, "mm")),'p_year':Number(dateFormat(periode, "yyyy"))})))
                        // console.log(submiting,defaultDataValue)
                        // // // console.log('save')
                        break;
                    case Appresources.BUTTON_LABEL.LABEL_UPDATE:
                        ...
                        dispatch(executeMAVH(_.merge(defaultDataValue,{'p_month':Number(dateFormat(periode, "mm")),'p_year':Number(dateFormat(periode, "yyyy"))})))
                        // console.log(_.merge(defaultDataValue,{'p_month':Number(dateFormat(periode, "mm")),'p_year':Number(dateFormat(periode, "yyyy"))}))
                        break;
                    default:
                        break;
                }
    ...
    const defaultDataValue = { vehiclecode: units[0] }
