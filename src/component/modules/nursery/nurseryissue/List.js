/*=============================================================================
 ...
 |  
 |          
 |  Description:  Nursery Issue  List
 |
 *===========================================================================*/
 ...
 const List = ({ data, datadetail, queryPageSize, isloading, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
     // console.log(data[0])
     ...
     const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.sivcode))
                    await dispatch(fetchDatadetail(row.original.sivcode,
                        // async () => {
                ...
                await prepareEditDataMasterDetail(row)
                navigate(`./edit/${encodeURIComponent(row.original.sivcode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.sivcode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
