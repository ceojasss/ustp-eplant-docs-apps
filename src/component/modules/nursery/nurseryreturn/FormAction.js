...
const ROUTES = '/nursery/Trx/nurseryreturn'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch nurseryreturn')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch nurseryreturndetail')

    const response = await eplant.get(`${ROUTES}/detail?returncode=${params}`)

    console.log('done fetch nurseryreturndetail')
