                ...
                setTimeout(() => {
                    ...
                    if (_.isUndefined(initialValues['v_url_preview'])) {
                        // if (_.isEmpty(submitdata['inputgrid'])){
                            dispatch(fetchLovUrlPreview('NRN', initialValues['process_flag'], initialValues['returncode']))
                    }
                }, 0.1);
            } else {
 ...
 */                    break;
    ...
    let buttonReport
        ...
        if (actionlabel == Appresources.BUTTON_LABEL.LABEL_UPDATE && (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('rpt_nursery_return.rdf') : _.includes(initialValues['v_url_preview'], 'rpt_nursery_return.rdf')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_NURSERY_RETURN.rdf') : _.includes(initialValues['v_url_preview'], 'RPT_NURSERY_RETURN.rdf')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_NURSERY_RETURN.RDF') : _.includes(initialValues['v_url_preview'], 'RPT_NURSERY_RETURN.RDF'))) 
    {
        // console.log('1')
        buttonReport = [{
 ...
 */
    ...
    yup.addMethod(yup.array, 'destination', function (fields) {
        return this.test('description', '', function (array, v) {
