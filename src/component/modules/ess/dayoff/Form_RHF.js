...
const Form = ({title,actions,formComps,initialValues,postdata,formValidationSchema,submitdata,actionlabel,resetTrx,}) => {
  ...
  const submitHandler = () =>
    formRefs.current((rowid, data, object) => {
      /*
       * 1. Set post Data.
      ...
      let postObject =
        actionlabel === NEWS ? object : { ROWIDS: rowid, ...object };

      dispatch(
        DialogConfirmation(
          actionlabel === Appresources.BUTTON_LABEL.LABEL_SAVE ? SAVE : UPDATE,
          null,
          rowid,
          data
        )
...
const customschema = (formValidationSchema, formComps, methods) => {
  ...
  yup.addMethod(yup.number, "setSisaDayOff",
        function setSisaDayOff() {
            return this.test((value, values) => {
      // const saldo = getValues("saldo");
      // const jmlcuti = getValues("jmlcuti");
      let tglawal = getValues('tglawal'); 
    ...
    //   }
      ...
      // Menambahkan jumlah cuti ke tanggal akhir
    //   tglakhir.setDate(tglakhir.getDate() + parseInt(jmlcuti) - 1); 
  
    //   console.log("tglawal", tglawal);
    //   console.log("tglakhir",tglakhir);
  
    //   if (_.isNaN(saldo - jmlcuti)) {
    //     return "invalid";
    //   }
   
      // setValue("tglakhir", tglakhir.toLocaleDateString(), { shouldDirty: true });
                // console.log()
                // const a = saldo - jmlcuti
                console.log(getValues('saldo'))
        if (!_.isUndefined(getValues("saldo")) && !_.isUndefined( getValues("jmlcuti"))){
          // console.log('1',new Date(tglawal.setDate(tglawal.getDate() + parseInt(getValues("jmlcuti")) - 1)))
            setValue("sisa", getValues("saldo") - getValues("jmlcuti"), { shouldDirty: true });
            setValue("tglakhir", new Date(tglawal.setDate(tglawal.getDate() + parseInt(getValues("jmlcuti")) - 1)), {shouldDirty: true})
        }
  ...
  customSchema = formValidationSchema.concat(
    yup.object().shape({
      sisa: yup.number().setSisaDayOff(),
      // tglakhir: yup.date().setSisaDayOff(),
