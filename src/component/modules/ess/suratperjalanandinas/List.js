...
const List = ({ data, datadetail, isloading, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid, actions, selectedValue, modalResult }) => {
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case PROCESS:
                //                dispatch(DialogConfirmationOnly(APPROVE, `Approve Transaksi ${row.original.empcode} ?`, row))

                //                // console.log('hehehe')
                dispatch(ConfirmationApprovedWithDate(`Payment Voucher Approved ${row.original.empcode}`, 'Pilih Tanggal Approve', row.original))
                break;
            case UPDATE:
                ...
                if (!isDataExist(row.original.empcode))
                    await dispatch(fetchDatadetail(row.original.empcode,
                        // async () => {
    ...
    const renderRowSubComponent = React.useCallback(
        ({ row, rowProps, visibleColumns }) => (
            <SubRowAsync
                /**
                 * ! ganti empcode sesuai dengan id nya
                 */
                row={getRowData(row)}
                rowProps={rowProps}
