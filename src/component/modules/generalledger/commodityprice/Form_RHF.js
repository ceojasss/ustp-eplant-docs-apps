...
const Forms = ({ title, periode, units, actions, formComp, formComps, initialValues, postdata, formValidationSchema,
    ...
    const generateData = async (cb) => {
        ...
        const key_doc = methods.control._formValues.productcode
        const progressdate = methods.control._formValues.tdate
        ...
        if (!_.isEmpty(key_doc) && _.size(methods.control._formValues.inputgrid) === 0) {
            ...
            await dispatch(fetchDataGenerate(key_doc,progressdate, () => {
...
const customschema = (formValidationSchema, formComp, formComps, methods) => {
    ...
    yup.addMethod(yup.array, 'tdate', function (fields) {
        return this.test('tdate', 'tdate', function (array, val) {
            ...
            _.mapValues(val.originalValue, (v, index) => {
                ...
                if (_.isUndefined(v.tdate)) {
                    // errors.push(index)
                    setValue(`${this.path}[${[index]}].tdate`, getValues('tdate'), { shouldDirty: true })
                    // return;
                }
            })
            ...
            if (errors.length === 0) {
                ...
                // setValue(`${this.path}[${errors[0]}].tdate`, getValues('tdate'), { shouldDirty: true })
    ...
    customSchema = formValidationSchema
        .concat(
            yup.object().shape({
                inputgrid: yup.array()
                    .tdate().description().volcurrent().qtyreq()
                // tdate: yup.string().tdate(),
            }))
