/*=============================================================================
 ...
 |                - Bagas Dwi Putra
 |          
 |  Description:  List Transaksi Contract Progress
 |
 *===========================================================================*/
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.productcode))
                    await dispatch(fetchDatadetail(row.original.productcode,
                        // async () => {
                ...
                navigate(`./edit/${encodeURIComponent(row.original.productcode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.productcode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
