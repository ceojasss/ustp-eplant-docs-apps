...
const ROUTES = '/contract/Trx/contractprogress'
...
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch contractprogress')
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    console.log('run fetch contractprogressdetail')

    const response = await eplant.get(`${ROUTES}/detail?documentno=${params}`)

    console.log('done fetch contractprogressdetail')
...
export const fetchDataGenerate = (params, progressno, progressdate, callback) => async dispatch => {
    ...
    const response = await eplant.get(`${ROUTES}/crdata?agreementcode=${params.code}&progressno=${progressno}&progressdate=${format(progressdate, 'yyyymmdd')}`)
