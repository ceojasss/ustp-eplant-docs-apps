...
const Forms = ({ title, periode, units, actions, formComp, formComps, initialValues, postdata, formValidationSchema,
            ...
            if (!_.isEmpty(submitdata)) {
                ...
                setTimeout(() => {
                    ...
                    if (_.isUndefined(initialValues['v_url_preview'])) {
                        ...
                        // } else {
                        dispatch(fetchLovUrlPreview('CA', initialValues['process_flag'], initialValues['agreementcode']))
                        // }
                    }
                }, 0.1);
            } else {
    ...
    if (actionlabel == Appresources.BUTTON_LABEL.LABEL_UPDATE && (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('rpt_view_contract_agreement.rdf') : _.includes(initialValues['v_url_preview'], 'rpt_view_contract_agreement.rdf')) || (!_.isEmpty(urlpreview) ? _.get(urlpreview[0], 'v_url_preview').match('RPT_VIEW_CONTRACT_AGREEMENT.RDF') : _.includes(initialValues['v_url_preview'], 'RPT_VIEW_CONTRACT_AGREEMENT.RDF'))) {
        // console.log('1')
        buttonReport = [{
...
const customschema = (formValidationSchema, formComp, formComps, methods) => {
    ...
    customSchema = formValidationSchema
        .concat(
            yup.object().shape({
                authorization: yup.string().setAuthDate(),
            }))
