    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.requestcode))
                    await dispatch(fetchDatadetail(row.original.requestcode,
                        // async () => {
                ...
                navigate(`./edit/${encodeURIComponent(row.original.requestcode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.requestcode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
