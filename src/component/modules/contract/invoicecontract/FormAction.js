...
export const ROUTES = '/contract/Trx/invoicecontract'
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    console.log('done fetch invoicecontract')
...
export const fetchDatadetail = (params) => async dispatch => {
    console.log('run fetch invoicecontractdetail')

    const response = await eplant.get(`${ROUTES}/detail?invoicecode=${params}`)

    console.log('done fetch invoicecontractdetail')

    dispatch({ type: FETCH_LISTDATADETAIL, payload: response.data })
    // if (callback) callback()
}

export const fetchDataGenerate = (params, documentno, callback) => async dispatch => {
    // console.log('run fetch ',params,progressno)

    dispatch(DialogLoading())

    const response = await eplant.get(`${ROUTES}/crdata?agreementcode=${params}&documentno=${documentno}`)
