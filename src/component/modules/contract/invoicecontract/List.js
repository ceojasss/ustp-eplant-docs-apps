        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.documentno))
                    await dispatch(fetchDatadetail(row.original.invoicecode,
                        // async () => {
                ...
                navigate(`./edit/${encodeURIComponent(row.original.invoicecode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.invoicecode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
