...
const Forms = ({ title, periode, units, actions, formComp, formComps, initialValues, postdata, formValidationSchema,
    ...
    const generateData = async (cb) => {
        ...
        console.log(key_doc, progressdate, progressno, _.isNil(progressdate))

        if (!_.isEmpty(key_doc) && !_.isNil(progressdate) && _.size(methods.control._formValues.inputgrid) === 0) {
