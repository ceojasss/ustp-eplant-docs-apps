...
const ROUTES = "/infrastructure/setup/infrastructure";
export const fetchDataheader = (pageIndex, pageSize, search, date, callback) => async dispatch => {
    ...
    //    const response = await eplant.get(`/cashbank/Trx/infrastructure?page=${(_.isUndefined(pageIndex) ? 0 : pageIndex)}&size=${(_.isUndefined(pageSize) ? 10 : pageSize)}&search=${search}`)


    const response = await eplant.get(`${ROUTES}?page=${pageIndex}&size=${pageSize}&search=${search}`)

    console.log('done fetch infrastructure')
...
export const fetchDatadetail = (params, callback) => async dispatch => {
    console.log('run fetch infrastructuredetail')

    const response = await eplant.get(`${ROUTES}/detail?ifcode=${params}`)

    console.log('done fetch infrastructuredetail')
