...
const List = ({ data, datadetail, isloading, queryPageSize, queryPageIndex, pageCount, search, dateperiode, reducerid }) => {
    ...
    const rowClickHandler = async (action, row) => {
        ...
        switch (action) {
            case UPDATE:
                ...
                if (!isDataExist(row.original.ifcode))
                    await dispatch(fetchDatadetail(row.original.ifcode,
                        // async () => {
                ...
                await prepareEditDataMasterDetail(row)
                navigate(`./edit/${encodeURIComponent(row.original.ifcode)}`)
                break;
            case DELETE:
                // * set custom delete message     
                dispatch(DialogConfirmation(DELETE, `Kode Data : ${row.original.ifcode} Akan Dihapus`, row))
                break;
            default:
                break;
        }
    ...
    const renderRowSubComponent = React.useCallback(
        ({ row, rowProps, visibleColumns }) => (
            <SubRowAsync
                /**
                 * ! ganti ifcode sesuai dengan id nya
                 */
                row={getRowData(row)}
                rowProps={rowProps}
