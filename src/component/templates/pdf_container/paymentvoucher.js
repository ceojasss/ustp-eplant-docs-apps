...
import { AuthBox, CompanyLogo, debugBase64, multiText, pvDoc, strToObj } from './util';
...
const headerContent = (doc, hdata, Xleft, Ymove, callback) => {
    ...
    const namevendor = hdata.header_namevendor ? hdata.header_namevendor : ''
    ...
    doc.text('Mata Uang', Xleft + 148, Ymove + h1, { align: 'left' });

    doc.text('Cheque/Giro No.', Xleft + 2, Ymove + h2, { align: 'left' });
    doc.text('Kurs', Xleft + 92, Ymove + h2, { align: 'left' });
    doc.text('Tanggal', Xleft + 148, Ymove + h2, { align: 'left' });
    ...
    doc.text(hdata.header_currency, Xleft + 168, Ymove + h1, { align: 'left' });

    doc.text(`${ceknumber}`, Xleft + 32, Ymove + h2, { align: 'left' });
    doc.text(`${hdata.header_rate}`, Xleft + 115, Ymove + h2, { align: 'left' });
    doc.text(hdata.header_datecreated, Xleft + 168, Ymove + h2, { align: 'left' });
    ...
    doc.text('No Rekening', Xleft + 148, Ymove + h2, { align: 'left' });

    // Value
    doc.setFont('helvetica', 'normal');
    doc.text(`${paymentto} - ${namevendor}`, Xleft + 32, Ymove + h1, { align: 'left' });
...
export const paymentvoucher = (tkn, datareport, pdfPreviewRef) => {
    ...
    doc.autoTable({
        ...
        head: [
            ...
            { content: 'Uraian', rowSpan: 2, styles: { halign: 'center', valign: 'center', cellWidth: 73 } },
            { content: 'Jumlah', rowSpan: 2, styles: { halign: 'center', valign: 'center', cellWidth: 40 } }],
            [
    ...
    doc.setFont('helvetica', 'bold');
    doc.text("Total", doc.internal.pageSize.getWidth() - 50, startY, null, null, "right");
